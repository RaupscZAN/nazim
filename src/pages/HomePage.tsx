...
import { ArrowRight, ChevronRight, MapPin, Award, FileText, BookOpen, Mail } from 'lucide-react';
...
const HomePage: React.FC = () => {
  ...
  const awards = [
    { year: "2022", title: "Harvard University Muslim Alumni Foundation Award", description: "Outstanding contributions to Islamic finance education" },
    { year: "2019", title: "Islamic Finance Leadership Award", description: "Pioneering research in global Islamic finance" },
    { year: "2017", title: "Royal Award for Islamic Finance", description: "Presented by the King of Malaysia" },
  ];

  const videos = [
    { id: "zOkBZUTWQfY", title: "Lariba Presentation", location: "Pasadena, CA" },
  ...
  const featuredPapers = [
    {
      ...
      description: "Examines the transformation of Islamic finance through digital technologies and innovation.",
      category: "Books",
      link: "/research?type=Books"
    },
    {
      title: "14th Harvard University Forum on Islamic Finance",
      year: "2022",
    ...
    <div className="bg-cream-50">
        ...
        <div className="container-wide relative z-10 pt-20">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              ...
              <span className="block text-gold-400 italic">Education & Research</span>
            </h1>
            <p className="text-lg text-gray-300 mb-8 leading-relaxed max-w-2xl mx-auto">
              Dr. Syed Nazim Ali is a globally recognized scholar with over four decades of
              contributions to Islamic economics and finance, currently serving as Professorial
      ...
      <section className="py-12 bg-white border-b border-cream-300">
        <div className="container-wide">
          <p className="text-center text-sm text-slate-500 uppercase tracking-widest mb-8">Academic Affiliations</p>
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16">
            {affiliations.map((item, index) => (
              <div key={index} className="text-center">
                <h3 className="font-serif text-xl font-bold text-slate-800">{item.name}</h3>
      ...
      <section className="py-24 bg-white overflow-hidden">
        <div className="container-wide">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            {/* Content */}
            <div>
              <p className="text-burgundy-800 font-semibold tracking-widest uppercase text-sm mb-4">About</p>
              <h2 className="font-serif text-4xl md:text-5xl font-bold text-burgundy-800 mb-8">Dr. Syed Nazim Ali</h2>
              <p className="text-slate-700 leading-relaxed mb-5 text-base">
                A distinguished scholar and institution-builder who spent two decades at
                Harvard University, where he founded the <span className="text-burgundy-800">Harvard Islamic Finance Program</span>,
                established the <span className="text-burgundy-800">Islamic Finance Databank</span>, and launched the <span className="text-burgundy-800">Harvard
                University Forum on Islamic Finance</span>.
              </p>
              <p className="text-slate-500 leading-relaxed mb-10 text-base">
                Previously served as Research Professor and Director of the Research Division at the
                College of Islamic Studies, Hamad Bin Khalifa University (HBKU), Qatar Foundation, from
                2014 to 2025. His work has shaped global discourse on Islamic finance education and
                research.
              </p>
              ...
              <div className="flex gap-4 mb-10">
                <div className="text-center py-4 px-6 bg-cream-50 rounded-lg border border-cream-200 flex-1">
                  <div className="font-serif text-3xl font-bold text-burgundy-800">40<span className="text-gold-500 text-xl align-top">+</span></div>
                  <p className="text-xs text-slate-400 mt-1">Years Experience</p>
                </div>
                <div className="text-center py-4 px-6 bg-cream-50 rounded-lg border border-cream-200 flex-1">
                  <div className="font-serif text-3xl font-bold text-burgundy-800">50<span className="text-gold-500 text-xl align-top">+</span></div>
                  <p className="text-xs text-slate-400 mt-1">Publications</p>
                </div>
                <div className="text-center py-4 px-6 bg-cream-50 rounded-lg border border-cream-200 flex-1">
                  <div className="font-serif text-3xl font-bold text-burgundy-800">29</div>
                  <p className="text-xs text-slate-400 mt-1">Years at Harvard</p>
                </div>
              </div>
      ...
      <section className="py-20 bg-cream-100">
        <div className="container-wide">
          <div className="text-center mb-12">
            <p className="text-burgundy-800 font-medium tracking-wider uppercase mb-2">Recognition</p>
            <h2 className="font-serif text-4xl font-bold text-slate-800">Awards & Honors</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {awards.map((award, index) => (
              <div key={index} className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all border-l-4 border-gold-500">
                <div className="flex items-center mb-3">
                  <Award className="text-gold-500 mr-2" size={24} />
                  <span className="text-gold-600 font-bold text-lg">{award.year}</span>
                </div>
                <h3 className="font-serif text-lg font-bold text-slate-800 mb-2">{award.title}</h3>
      ...
      <section className="py-20 bg-white">
        <div className="container-wide">
          <div className="text-center mb-12">
            <p className="text-burgundy-800 font-medium tracking-wider uppercase mb-2">Research</p>
            <h2 className="font-serif text-4xl font-bold text-slate-800">Featured Papers</h2>
          </div>
            ...
            {featuredPapers.map((paper, index) => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-burgundy-800 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-gold-500 transition-colors">
                    <FileText className="text-white" size={24} />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span className={`text-xs font-semibold px-2 py-1 rounded ${
                        paper.category === 'Harvard' ? 'bg-red-100 text-red-700' :
                        paper.category === 'SOAS' ? 'bg-blue-100 text-blue-700' :
                        paper.category === 'LSE' ? 'bg-purple-100 text-purple-700' :
                        'bg-gold-100 text-gold-700'
                      }`}>{paper.category}</span>
                      <span className="text-xs font-medium text-slate-600">{paper.year}</span>
                    </div>
                    <h3 className="font-serif text-lg font-bold text-slate-800 mb-2 group-hover:text-burgundy-800 transition-colors">
                      {paper.title}
                    </h3>
                    <p className="text-slate-500 text-sm mb-3">{paper.description}</p>
                    <span className="text-xs text-slate-400">{paper.publisher}</span>
                    <div className="mt-3 inline-flex items-center text-burgundy-800 group-hover:text-burgundy-600 text-sm font-medium">
                      View in {paper.category} Collection <ChevronRight size={16} className="ml-1" />
                    </div>
                  </div>
                </div>
              </Link>
      ...
      <section className="py-20 bg-slate-900">
        <div className="container-wide">
          <div className="text-center mb-12">
            <p className="text-gold-400 font-medium tracking-wider uppercase mb-2">Media</p>
            <h2 className="font-serif text-4xl font-bold text-white">Featured Presentations</h2>
          </div>
        ...
        <div className="container-wide">
          <div className="bg-gradient-to-r from-burgundy-50 to-gold-50 rounded-2xl p-8 md:p-12 border border-burgundy-100">
            <div className="flex items-center justify-center mb-6">
              ...
              <h2 className="text-3xl font-serif font-bold text-burgundy-800">Upcoming Book</h2>
            </div>
            ...
            <div className="max-w-2xl mx-auto text-center">
              ...
              {!isSubscribed ? (
                <div className="bg-white rounded-xl p-6 shadow-sm border border-cream-200">
                  ...
                  <form onSubmit={handlePreRegistration} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                    <div className="flex-1">
                      <input
                        type="email"
      ...
      <section className="py-20 bg-burgundy-900">
        <div className="container-narrow text-center">
          <h2 className="font-serif text-4xl font-bold text-white mb-4">Let's Connect</h2>
          <p className="text-cream-300 text-lg mb-8 max-w-2xl mx-auto">
            For academic inquiries, speaking engagements, research collaborations, or media
            interviews, I welcome the opportunity to connect.
          </p>
          <Link
            to="/contact"
